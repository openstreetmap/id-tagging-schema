...
- ✅ **Proposal or Accepted**: A tag is considered established when it has history of being widely used by different mappers or is otherwise accepted by the OSM community. Factors include the tag's duration and frequency of use, whether its usage is increasing over time and its usage by mainstream data consumers, completing the [proposal process](https://wiki.openstreetmap.org/wiki/Proposal_process).
